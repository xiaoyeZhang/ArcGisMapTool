...
typedef NS_ENUM(NSInteger, AGSOGCAxisOrder) {
    ...
    AGSOGCAxisOrderNoSwap = 2          /*!< Order of x/y coordinates stays as-is. All swapping rules by OGC standards are ignored. */
};
...
typedef NS_ENUM(NSInteger, AGSErrorCode) {
    ...
    AGSErrorCodeGDBDomainAlreadyExists = 3079,                              /*!<  */
    AGSErrorCodeGDBGeometryTypeNotSupported = 3080,                         /*!<  */
...
typedef NS_ENUM(NSInteger, AGSPortalPrivilegeType) {
    ...
    AGSPortalPrivilegeTypePublishServerGPServices,  /*!< Allows organization members to publish non-hosted server geoprocessing services. Dependent on @c AGSPortalPrivilegeTypePublishServerServices.
                                                     @since 100.5 */
    AGSPortalPrivilegeTypePublishServerServices,    /*!< Allows organization members to publish non-hosted server services.
                                                     @since 100.5 */
    AGSPortalPrivilegeTypeUpdateItemCategorySchema  /*!< Allows organization member the ability to create and update the organization category schema.
                                                     @since 100.5 */
};
...
typedef NS_ENUM(NSInteger, AGSKMLRefreshMode) {
    ...
    AGSKMLRefreshModeOnInterval = 1,         /*!< Refresh every n milliseconds. Note, if refreshInterval is 0 when the mode is set to onInterval, it will automatically update the refreshInterval to 500 milliseconds. */
...
typedef NS_ENUM(NSInteger, AGSKMLViewRefreshMode) {
    ...
    AGSKMLViewRefreshModeOnStop = 1,        /*!< Refresh the file n milliseconds after movement stops, where n is specified by viewRefreshTime. Note, if viewRefreshTime is 0 when the mode is set to onStop, it will automatically update the viewRefreshTime to 500 milliseconds. */
