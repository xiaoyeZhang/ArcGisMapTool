...
/** The path to a directory on the device where the local basemap file is located.
 This property should be set (along with `AGSGenerateOfflineMapParameters#referenceBasemapFilename`) in order to to use a basemap which is already on the device, rather than downloading.
 ...
 Note that this property is ignored if `AGSGenerateOfflineMapParameters#includeBasemap` is `false`.
 @since 100.5
 */
