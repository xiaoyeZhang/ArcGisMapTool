...
/** Initialize the task with the provided portal item. The item must represent a webmap (item type should be @c AGSPortalItemTypeWebMap) either on ArcGIS Online or an on-premises ArcGIS Portal.
...
/** Portal item specifying the map to take offline. The item must represent a webmap (item type should be @c AGSPortalItemTypeWebMap) either on ArcGIS Online or an on-premises ArcGIS Portal.
 @since 100.1
 */
...
/** A convenience method to create the AGSDownloadPreplannedOfflineMapParameters with default values, appropriate for taking the specified map area offline.
 The default parameters will be populated using the values in the online map's offline settings (if present).
...
/** Provides overrides for `AGSGenerateOfflineMapParameters`.
 Upon successful completion, a `AGSGenerateOfflineMapParameterOverrides` instance will be populated reflecting the values in the `AGSGenerateOfflineMapParameters` parameters. The overrides may be inspected and modified to change the offline data before creating a job with `AGSOfflineMapTask#generateOfflineMapJobWithParameters:parameterOverrides:downloadDirectory:`.
